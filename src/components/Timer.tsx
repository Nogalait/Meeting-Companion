...
export function Timer({ initialSeconds }: TimerProps) {
  ...
  return (
    <div className={cn(
      "flex flex-wrap sm:flex-nowrap items-center justify-center gap-2 sm:gap-3.5 px-3 py-1.5 sm:px-4 sm:py-2 rounded-full border shadow-sm transition-colors max-w-full",
      isLast30s ? "bg-red-50 border-red-200 animate-pulse text-red-700" : "bg-zinc-100 border-zinc-200"
    )}>
      <Clock className={cn("hidden sm:block w-4.5 h-4.5", isLast30s ? "text-red-500" : "text-zinc-400")} />
      ...
      {isEditing ? (
        <div className="flex items-center gap-1 sm:gap-2">
          <input 
            ...
            className="w-12 sm:w-16 bg-white border border-zinc-300 rounded px-1 sm:px-2 py-0.5 text-sm sm:text-base outline-none focus:border-zinc-500"
            min="1"
            autoFocus
            onBlur={handleSave}
        ...
        <div 
          ...
          className={cn(
            "font-mono text-sm sm:text-base font-medium cursor-pointer w-10 sm:w-14 text-center transition-colors",
